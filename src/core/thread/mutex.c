...
int mutex_init(pthread_mutex_t *lock, pthread_mutexattr_t *attr) {
	...
	if (result) {
		mclog_pedantic("Could not initialize the mutex. {pthread_mutex_init = %i / error = %s}", result, strerror_r(errno, MEMORYBUF(1024), 1024));
	}
	return result;
#else
...
int mutex_lock(pthread_mutex_t *lock) {
	...
	if (result) {
		mclog_options(M_LOG_PEDANTIC | M_LOG_STACK_TRACE, "Could not lock the mutex. {pthread_mutex_lock = %i / error = %s}", result, strerror_r(errno, MEMORYBUF(1024), 1024));
	}
	return result;
#else
...
int mutex_unlock(pthread_mutex_t *lock) {
	...
	if (result) {
		mclog_options(M_LOG_PEDANTIC | M_LOG_STACK_TRACE, "Could not unlock the mutex. {pthread_mutex_unlock = %i / error = %s}", result, strerror_r(errno, MEMORYBUF(1024), 1024));
	}
	return result;
#else
...
int mutex_destroy(pthread_mutex_t *lock) {
	...
	if (result) {
		mclog_pedantic("Could not destroy the mutex. {pthread_mutex_destroy = %i / error = %s}", result, strerror_r(errno, MEMORYBUF(1024), 1024));
	}
	return result;
#else
